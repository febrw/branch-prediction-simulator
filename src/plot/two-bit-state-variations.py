...
f = open("../jsons/two-bit-start-states.json")
...
df.groupby('start_state')['missprediction_rate']
...
for i, size in enumerate(df['start_state'].unique()):
    # Get the missprediction rates for the current table size
    y_values = df[df['start_state'] == size]['missprediction_rate'].values
...
# Add a legend
ax.legend(title='Start State', fontsize=14)
plt.title("Bimodal Predictor with varying start states", fontsize=26)
# Show the plot
